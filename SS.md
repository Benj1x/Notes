...
Pumping Lemma (for CFL (context free langauges)) is used to prove that a language is NOT context free
A context Free Langauge is a language generated by some Context Freee Grammar. The set of all CFL is identical to the set of languages accepted by Pushdown Automata
...
where, `à = {V \cup \Sigma}*` and `A ∈ V`

If A is a Context Free Language, then, A has a Pumping Length 'P' such that any string 'S', where `|S| => P` may be divided into 5 pieces `S = uvxyz` (so split the string into 5) such that the following conditions must be true:

<span style="color:red">(1) `u v^i x y^i z is in A for every i >= 0` (if you raise v and y to the power of i where i is bigger than or equal to zero, the string you get, after doing this, should also belong to the language A)</span>

<span style="color:red">(2) `|vy| > 0` (the length of v and y should be greater than zero)</span>

<span style="color:red">(3) `|v x y| <= P` (the length of v x y should be less than or equal to P)</span>

Remember that uvxyz aren't strings, but parts of the string, after doing this, all of the above conditions should be true, have to be true, for the langauge to be context free.

In order to prove that a Language is Not Context Free using Pumping Lemma (for CFL) follow the steps given below: (This proof uses contradiction)

* Assume that A is Context Free
* It has to have a Pumping Length (P) (Since we assume that A is context free)
* All strings longer than P can be pumped `|S| => P` (Any string, greater than or equal to P can be pumped)
* Now find a string 'S' in A such that `|S| => P` (from A, take a string 'S', that is greater than or equal to P)
* Divide 'S' into the five different parts (`uvxyz`)
* Show that `u v^i x y^i z \notin A` for some `i` (So show that the string uvxyz (the string `S`) it is NOT in A, when v and y has been repeated `i` times)
* Then consider the ways that 'S' can be divided into `uvxyz` (repeat it for all combinations of uvxyz)
* Show that none of these can satisfy all the 3 pumping conditions <span style="color:red">(Highlighted above)</span> at the same time
* S cannot be pumped == contradiction (If S cannot be pumped, we have proved that A is not a CFL by contradiction)

Some examples of Pumping Lemma for CFL:

Example 1:

Here, the langauge `L = {a^N b^N c^N | N => 0}` so, the language is N A's, N B's, N C's where N is any number (but the same for all letters). 

![Pumping Lemma for CFL 1.1](/imgs/SS/PLemmaForCFL1p1.png)

From case 1, we can see it is not in L. So we already know it is not context free, but we'll do a second case, just for good measure.

![Pumping Lemma for CFL 1.2](/imgs/SS/PLemmaForCFL1p2.png)

This case certinaly does not hold for L either, L is not context free

'Straddles' reffers to if for example `u` of the string, lies on both boundaries of a border `'`
![Pumping Lemma for CFL 2.1](/imgs/SS/PLemmaForCFL2p1.png)

From case 1, we can see it is not in L. So we already know it is not context free, but we'll do a second case, just for good measure.

![Pumping Lemma for CFL 2.2](/imgs/SS/PLemmaForCFL2p2.png)

This does not hold for L either

![Pumping Lemma for CFL 2.3](/imgs/SS/PLemmaForCFL2p3.png)

This does not hold for L either

![Pumping Lemma for CFL 2.4](/imgs/SS/PLemmaForCFL2p4.png)

Not all three conditions could be satisfied, only 2 and 3, therefore L is not context free
